...
def fetch_students():
    print(f"Tentando conectar a: {MACHINE_B_URL}/students") 
    try:
        resp = requests.get(f"{MACHINE_B_URL}/students", timeout=5)
        resp.raise_for_status()
        
        data = resp.json()
        print(f"Dados recebidos da API (primeiros 500 caracteres): {str(data)[:500]}") 

        if isinstance(data, dict) and 'users' in data and isinstance(data['users'], list):
            student_list = data['users']
        else:
            student_list = []
        ...
        for aluno in student_list:
            if not isinstance(aluno, dict):
                print(f"Aviso: Item de aluno malformado (não é um dicionário) ignorado: {aluno}")
                continue
            ...
            if "school_grades" not in aluno or not isinstance(aluno["school_grades"], dict):
                ...
                for n in ["AVA 1", "AVA 2", "AVA 3"]:
                    try:
                        value = float(aluno["school_grades"].get(n, 0.0))
                    except (ValueError, TypeError):
                        value = 0.0
                    aluno["school_grades"][n] = value
            
            processed_data.append(aluno)

        return processed_data
    except requests.RequestException as e:
        messagebox.showerror("Erro de Conexão", f"Não foi possível conectar à Máquina B (API):\n{e}")
        print(f"ERRO DE REQUISIÇÃO: {e}") 
        return []

def update_student_on_machine_b(student):
    try:
        resp = requests.post(f"{MACHINE_B_URL}/students/update", json=student, timeout=5)
        resp.raise_for_status()
    except requests.RequestException as e:
        messagebox.showerror("Erro", f"Falha ao atualizar notas na Máquina B:\n{e}")

def get_students_by_class(class_name):
    data = fetch_students()
    class_suffix = class_name[-1]
    return [aluno for aluno in data 
            if isinstance(aluno, dict) 
            and aluno.get("student_class")
            and aluno.get("student_class").endswith(class_suffix)]

def calculate_average(grades):
    try:
        return round((grades.get("AVA 1", 0) + grades.get("AVA 2", 0) + grades.get("AVA 3", 0)) / 3, 2)
...
def open_student_details(student, refresh_callback, parent_window):
    ...
    details_win.title(f"Detalhes - {student.get('name', 'Aluno')}")
    ...
    customtkinter.CTkLabel(details_win, text=f"Nome: {student.get('name', 'N/A')}", font=("Arial", 14)).pack(pady=5)
    customtkinter.CTkLabel(details_win, text=f"E-mail: {student.get('email', 'N/A')}", font=("Arial", 14)).pack(pady=5)
    customtkinter.CTkLabel(details_win, text=f"Turma: {student.get('student_class', 'N/A')}", font=("Arial", 14)).pack(pady=5)
    ...
    for n in ["AVA 1", "AVA 2", "AVA 3"]:
        ...
        initial_grade = student["school_grades"].get(n, 0.0)
        grade_vars[n] = customtkinter.StringVar(value=str(initial_grade))
    ...
    def save_grades():
        ...
        update_student_on_machine_b(student)
        messagebox.showinfo("Sucesso", f"As notas de {student.get('name', 'Aluno')} foram atualizadas.")
        details_win.destroy()
        refresh_callback()
...
def classes_window(previous_window):
    ...
    def mostrar_turma(nome):
        ...
        print(f"Alunos encontrados para {nome}: {len(alunos)}")
        ...
        for aluno in alunos:
            grades = aluno.get("school_grades", {"AVA 1": 0.0, "AVA 2": 0.0, "AVA 3": 0.0})
